...
irqreturn_t kbd_interrupt_handler(int irq_no, void * dev_id){
	struct kbd * my_kbd = (struct kbd*) dev_id;
	printk(KERN_DEBUG "keyboard pressed");
...
static int kbd_init(void)
{
	...
	if (!request_region(I8042_STATUS_REG, KBD_NR_MINORS, MODULE_NAME)){
		return -ENODEV;
	}
	if (!request_region(I8042_DATA_REG, KBD_NR_MINORS, MODULE_NAME)){
		return -ENODEV;
	}
...
out_release_regions:
	release_region(I8042_STATUS_REG, KBD_NR_MINORS);
	release_region(I8042_DATA_REG, KBD_NR_MINORS);
